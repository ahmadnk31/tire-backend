    ...
    try {
        ...
        for (let i = 0; i < results.length; i++) {
            try {
                let insertedRow;
                if (type === 'product') {
                    [insertedRow] = await index_1.db.insert(schema_1.products).values(results[i]).returning();
                }
                else if (type === 'user') {
                    [insertedRow] = await index_1.db.insert(schema_1.users).values(results[i]).returning();
                }
                else if (type === 'category') {
                    [insertedRow] = await index_1.db.insert(schema_1.categories).values(results[i]).returning();
                }
