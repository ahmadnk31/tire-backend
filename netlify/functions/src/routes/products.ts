...
import { eq, and, like, gte, lte, desc, asc, or, count, SQL, inArray, ilike } from 'drizzle-orm';
import { db } from '../db';
...
router.get('/', advancedSearchValidation, handleValidationErrors, async (req: express.Request, res: express.Response) => {
  try {
    ...
    if (category && category !== 'all') {
      ...
      console.log('[DEBUG] Decoded category filter:', category, '->', decodedCategory);
      ...
      const categoryResults = await db
        ...
        .from(categories)
        .where(or(
          eq(categories.name, decodedCategory as string),
          eq(categories.slug, decodedCategory as string)
        ));
